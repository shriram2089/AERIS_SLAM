...
class SerialOdomNode(Node):
    ...
    def timer_callback(self):
        ...
        try:
            ...
            # if line:
            #     self.get_logger().info(f"Serial raw: {line}")  # <-- prints whatever Arduino sends
        ...
        if motion is not None:
            ...
            # self.theta_fused = ((1-self.alpha) * self.theta_enc) + ((self.alpha) * self.theta_imu)
    ...
    def publish_transforms(self):
        ...
        q = euler2quat(0, 0, math.pi/2 )  # roll=0, pitch=0, yaw=90deg
        tf2.transform.rotation.x = q[1]    # transforms3d gives (w, x, y, z)
        tf2.transform.rotation.y = q[2]
        tf2.transform.rotation.z = q[3]
        tf2.transform.rotation.w = q[0]
    ...
    def publish_map_to_odom(self):
        ...
        # q = euler2quat(0,0, 0)  # roll=0, pitch=0, yaw=90deg
        q = euler2quat(0,0, 0 )  # roll=0, pitch=0, yaw=90deg
        
        tf4.transform.rotation.x = q[1]    # transforms3d gives (w, x, y, z)
        tf4.transform.rotation.y = q[2]
        tf4.transform.rotation.z = q[3]
        tf4.transform.rotation.w = q[0]
