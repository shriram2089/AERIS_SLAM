...
class SerialOdomNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("Sent odometry reset command to ESP32")
        # self.ser = serial.Serial('/dev/controller', 115200, timeout=1)
        ...
        self.create_subscription(Twist, '/robot2/cmd_vel', self.cmd_vel_callback, QoSProfile(depth=10))
    ...
    def cmd_vel_callback(self, msg):
        ...
        # Debug: confirm callback and serial message
        self.get_logger().info(f"[DEBUG] Writing to serial: MODE={self.mode_sent}, CMD={linear:.2f},{angular:.2f}")
    ...
    def timer_callback(self):
        ...
        # Convert to rad/s and apply scale correction (tune if needed)
        ...
        # gyr = np.array([math.radians(raw['gyro'][i] - self.gyro_bias[i]) * scale_factor for i in range(3)])
        ...
        #     f"GYR: {gyr[0]:.3f}, {gyr[1]:.3f}, {gyr[2]:.3f} rad/s | "
        #     f"ACC: {acc[0]:.3f}, {acc[1]:.3f}, {acc[2]:.3f}"
        # )
        
        # self.get_logger().info(f"IMU yaw: {yaw:.3f} rad | {math.degrees(yaw):.2f}°")
        
        
        self.current_theta = yaw  # Invert yaw
