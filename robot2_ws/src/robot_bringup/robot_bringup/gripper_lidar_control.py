...
class GripperLidarController(Node):
    def __init__(self):
        ...
        self.ser = serial.Serial('/dev/arduino_lidar', 115200, timeout=1)
        ...
        # Subscribers
        self.subscription = self.create_subscription(String, '/robot2/gripper_cmd', self.cmd_callback, 10)
        self.speed_subscription = self.create_subscription(String, '/robot2/lidar_speed', self.speed_callback, 10)

        # Publisher for LiDAR angle
        self.lidar_pub = self.create_publisher(Int16, '/robot2/lidar_angle', 10)
