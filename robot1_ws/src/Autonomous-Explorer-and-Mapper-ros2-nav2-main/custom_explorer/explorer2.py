...
class ExplorerNode(Node):
    def __init__(self):
        ...
        # TF buffer/listener to check transforms
        ...
        # Wait for Nav2 server before starting exploration timer
        self.get_logger().info("Waiting for Nav2 action server...")
        self.nav_to_pose_client.wait_for_server()
        self.get_logger().info("Nav2 action server is ready!")

        # Start exploration timer only after Nav2 is ready
    ...
    def navigate_to(self, x, y):
        ...
        # Check TF availability before sending goal
        if not self.is_transform_available():
            self.get_logger().warn("Transform not available (map -> base_link), retrying later...")
            return

        goal_msg = PoseStamped()
        goal_msg.header.frame_id = 'map'  # Use global map frame
