...
void processSerialCommand(String line) {
  ...
  if (mode == AUTO && line.startsWith("V:")) {
    ...
    if (vIdx != -1 && wIdx != -1) {
      ...
      // Convert linear velocity to motor command
      fr = br = -constrain(int((v_r * 60.0 / (2*PI*WHEEL_RADIUS)) * 10), -1000, 1000); // Right inverted
      fl = bl =  constrain(int((v_l * 60.0 / (2*PI*WHEEL_RADIUS)) * 10), -1000, 1000);  // Left normal
...
//    dc.ddsm_ctrl(4,  test_pwm, 2); bl =  test_pwm; // BL normal
  ...
  // Read RPMs
  float rpm_fr = dc.speed_data; dc.ddsm_ctrl(1, fr, 2); delay(1);
  float rpm_br = -dc.speed_data; dc.ddsm_ctrl(2, br, 2); delay(1);
  float rpm_fl = -dc.speed_data; dc.ddsm_ctrl(3, fl, 2); delay(1);
  float rpm_bl = dc.speed_data; dc.ddsm_ctrl(4, bl, 2); delay(1);
...
//    Serial.println("=== Motor + Odom Status ===");
//    Serial.printf("RPMs -> FR: %.2f, BR: %.2f, FL: %.2f, BL: %.2f\n", rpm_fr, rpm_br, rpm_fl, rpm_bl);
//    Serial.printf("Linear velocities -> v_fr: %.3f, v_br: %.3f, v_fl: %.3f, v_bl: %.3f\n", v_fr, v_br, v_fl, v_bl);
//    Serial.printf("v_left: %.3f, v_right: %.3f, vx: %.3f, omega: %.3f\n", v_left, v_right, vx, omega);
